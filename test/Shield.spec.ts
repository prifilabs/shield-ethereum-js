...
describe("Shield", function () {
  ...
  describe("Deployment", function () {
    it("Should deploy", async function () {
      const { factory, alice, bob, mallory } = await loadFixture(
    ...
    it("Should set the admin assignments", async function () {
      ...
      for (let f of [
        ...
        // console.log(shield.contract.interface.getSighash(f));
        expect(await shield.getAssignment(shield.contract, f)).to.deep.equal([
          ["admin"],
        ]);
      }
    });
  ...
  describe("Admin", function () {
    ...
    it("Should add a rule", async function () {
      ...
      const credentials = await shield.createCredentialsForAddRule(
        label,
        policy
      );
    ...
    it("Should set an assignment", async function () {
      ...
      const f = "addRoles";
      const label = "new-rule";
      const credentials = await shield.createCredentialsForAssignRule(
        shield.contract,
        f,
        label
      );
    ...
    it("Should validate multiple approvals", async function () {
      ...
      const credentials = await bobShield.createCredentialsForAddRoles(
        newRoles
      );
